
import os

import resend
from dotenv import load_dotenv


# ============================================================
# LOAD ENVIRONMENT VARIABLES
# ============================================================

load_dotenv()


# ============================================================
# RESEND CONFIGURATION
# ============================================================

RESEND_API_KEY = os.getenv("RESEND_API_KEY")

RESEND_FROM_EMAIL = os.getenv(
    "RESEND_FROM_EMAIL",
    "onboarding@resend.dev",
).strip()


# ============================================================
# VALIDATE RESEND CONFIGURATION
# ============================================================

if not RESEND_API_KEY:
    raise RuntimeError(
        "RESEND_API_KEY environment variable is not configured."
    )

if not RESEND_FROM_EMAIL:
    raise RuntimeError(
        "RESEND_FROM_EMAIL environment variable is not configured."
    )


# ============================================================
# CONFIGURE RESEND
# ============================================================

resend.api_key = RESEND_API_KEY


print(
    f"📧 Resend sender configured: {RESEND_FROM_EMAIL}"
)


# ============================================================
# SEND OTP EMAIL
# ============================================================

async def send_otp_email(
    to_email: str,
    otp: str,
):
    """
    Send email verification OTP using Resend.
    """

    # --------------------------------------------------------
    # NORMALIZE VALUES
    # --------------------------------------------------------

    to_email = str(to_email).strip().lower()
    otp = str(otp).strip()


    # --------------------------------------------------------
    # VALIDATE VALUES
    # --------------------------------------------------------

    if not to_email:
        raise ValueError(
            "Recipient email address is required."
        )

    if not otp:
        raise ValueError(
            "OTP is required."
        )


    # --------------------------------------------------------
    # CREATE EMAIL
    # --------------------------------------------------------

    params: resend.Emails.SendParams = {

        "from": RESEND_FROM_EMAIL,

        "to": [
            to_email
        ],

        "subject": "PredictHub Email Verification",

        "html": f"""
<!DOCTYPE html>

<html lang="en">

<head>

    <meta charset="UTF-8">

    <meta
        name="viewport"
        content="width=device-width, initial-scale=1.0"
    >

    <title>PredictHub Email Verification</title>

</head>

<body
    style="
        margin:0;
        padding:0;
        background:#0f172a;
        font-family:Arial,Helvetica,sans-serif;
    "
>

    <div
        style="
            max-width:600px;
            margin:40px auto;
            padding:20px;
        "
    >

        <div
            style="
                background:#111827;
                border:1px solid #1e293b;
                border-radius:16px;
                padding:35px;
                color:#ffffff;
            "
        >

            <!-- BRAND -->

            <h1
                style="
                    margin:0 0 10px;
                    font-size:28px;
                    color:#60a5fa;
                "
            >
                PredictHub
            </h1>

            <p
                style="
                    color:#94a3b8;
                    font-size:15px;
                    margin:0 0 30px;
                "
            >
                Machine Learning Prediction Platform
            </p>


            <!-- TITLE -->

            <h2
                style="
                    margin:0 0 10px;
                    font-size:22px;
                    color:#ffffff;
                "
            >
                Verify your email
            </h2>


            <!-- MESSAGE -->

            <p
                style="
                    color:#cbd5e1;
                    line-height:1.6;
                    font-size:15px;
                "
            >
                Thank you for registering with PredictHub.
                Use the verification code below to verify
                your email address.
            </p>


            <!-- OTP BOX -->

            <div
                style="
                    margin:30px 0;
                    padding:20px;
                    text-align:center;
                    background:#020617;
                    border:1px solid #334155;
                    border-radius:12px;
                "
            >

                <div
                    style="
                        font-size:12px;
                        color:#64748b;
                        margin-bottom:8px;
                        text-transform:uppercase;
                        letter-spacing:2px;
                    "
                >
                    Verification Code
                </div>

                <div
                    style="
                        font-size:36px;
                        font-weight:bold;
                        letter-spacing:10px;
                        color:#60a5fa;
                    "
                >
                    {otp}
                </div>

            </div>


            <!-- EXPIRATION -->

            <p
                style="
                    color:#94a3b8;
                    font-size:14px;
                    line-height:1.6;
                "
            >
                This verification code will expire in

                <strong style="color:#ffffff;">
                    5 minutes
                </strong>.
            </p>


            <!-- SECURITY -->

            <p
                style="
                    color:#64748b;
                    font-size:13px;
                    margin-top:30px;
                    line-height:1.6;
                "
            >
                If you did not request this verification code,
                you can safely ignore this email.
            </p>


            <!-- DIVIDER -->

            <hr
                style="
                    border:0;
                    border-top:1px solid #1e293b;
                    margin:30px 0;
                "
            >


            <!-- FOOTER -->

            <p
                style="
                    margin:0;
                    color:#475569;
                    font-size:12px;
                "
            >
                © 2026 PredictHub. All rights reserved.
            </p>

        </div>

    </div>

</body>

</html>
""",
    }


    # ========================================================
    # SEND EMAIL ASYNCHRONOUSLY
    # ========================================================

    try:

        print(
            f"📨 Sending OTP email to: {to_email}"
        )

        print(
            f"📤 Sending from: {RESEND_FROM_EMAIL}"
        )


        # IMPORTANT:
        # FastAPI async function -> Resend async method

        response = await resend.Emails.send_async(
            params
        )


        print(
            "✅ OTP email sent successfully:",
            response
        )


        return response


    except Exception as error:

        print(
            "❌ Resend email error:",
            repr(error)
        )

        raise

